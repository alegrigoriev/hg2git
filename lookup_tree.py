...
class lookup_tree:
	...
	def get_node(self, path, match_full_path=False, add_if_missing=False):
		...
		while True:
			...
			split = path.split(self.separator, 1)
			...
			def __next__(self):
				...
				while True:
					if self.child_iter is None:
						...
						if t[0].endswith(self.separator):
							continue
...
class path_tree(lookup_tree):
	...
	@staticmethod
	def compare(tree1, tree2, path=None):
		if tree1 is tree2:
			return

		if tree2 is None:
			if path is None:
				path = tree1.empty_string
			# Tree2 is deleted. Delete all sub-items and delete this item
			for key, node in tree1.dict.items():
				yield from type(tree1).compare(node, None, path + tree1.separator + key)

			yield (path, tree1, tree2)
			return

		if path is None:
			path = tree2.empty_string

		if tree1 is None:
			# Tree2 is added. Add this item and add its sub-items
			yield (path, tree1, tree2)
			for key, node in tree2.dict.items():
				yield from type(tree2).compare(None, node, path + tree2.separator + key)
			return

		iter1 = iter(sorted(tree1.dict.items(), key=lambda t: t[0]))
		iter2 = iter(sorted(tree2.dict.items(), key=lambda t: t[0]))

		NoneNone = (None, None)
		key1, item1 = next(iter1, NoneNone)
		key2, item2 = next(iter2, NoneNone)

		while item1 is not None or item2 is not None:
			if item2 is None or (key1 is not None and key1 < key2):
				yield from type(item1).compare(item1, None, path + item1.separator + key1)
				key1, item1 = next(iter1, NoneNone)
				continue

			if item1 is None or (key2 is not None and key2 < key1):
				yield from type(item2).compare(None, item2, path + item2.separator + key2)
				key2, item2 = next(iter2, NoneNone)
				continue

			yield from type(item2).compare(item1, item2, path + item2.separator + key2)
			key1, item1 = next(iter1, NoneNone)
			key2, item2 = next(iter2, NoneNone)
			continue

		if tree1.object is not tree2.object:
			yield (path, tree1, tree2)
		return

class bytes_path_tree(path_tree):
	separator = b'/'
	empty_string = b''
