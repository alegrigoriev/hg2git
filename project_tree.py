		...
		for t in difflist:
			...
			if obj1 is None:
				...
				if format_str1 != format_str2:
					print("WARNING: Formatting for file %s in branch %s changed" % (path, branch.name),file=self.log_file)
...
class project_branch(dependency_node):
	...
	def __init__(self, proj_tree:project_history_tree, branch_map, workdir:Path):
		...
		for file in self.cfg.inject_files:
			if file.path and file.branch.match(self.name):
		...
		if refname and refname in proj_tree.append_to_refs:
			...
			print('Found commit %s on previous refname "%s" to attach branch "%s"'
				%(info.commit, refname, self.name), file=self.proj_tree.log_file)
	...
	def finalize_commit(self, rev_info):
			...
			if parent_rev.committed_git_tree == rev_info.staged_git_tree and parent_rev.committed_git_tree != self.initial_git_tree:
				...
				if merged_to_parent_rev is not None and \
					...
					print("FAST FORWARD: Merge of %s;r%s to %s;r%s"
						% (parent_rev.branch.name, parent_rev.rev,
		...
		if need_commit:
			...
			commit_str = "\nCOMMIT:%s REF:%s BRANCH:%s;%s\n" % (commit, self.refname, self.name, rev_info.rev)
	...
	def ignore_file(self, path):
		...
		if ignore is None:
			ignore = self.cfg.ignore_files.fullmatch(path)
		return ignore
	...
	def preprocess_blob_object(self, obj, path):
		...
		if fmt is not None:
			if obj.git_attributes.get('formatting') != fmt.get_format_tag():
				obj = obj.make_unshared()
				obj.git_attributes['formatting'] = fmt.get_format_tag()
	...
	def finalize_deleted(self, rev, sha1):
		...
		if refname:
			print('Deleted revision %s on branch "%s" is preserved as refname "%s"'
				% (rev, self.name, refname), file=log_file)
		else:
			print('Deleted revision %s on branch "%s" not merged to any path or mapped to refname'
				% (rev, self.name), file=log_file)
		return

	def delete(self, revision, quiet=False):
		...
		if quiet:
			print('Branch "%s" merged back to revision %s\n' %
					(self.name, revision.rev), file=self.proj_tree.log_file)
		else:
			print('Branch "%s" deleted at revision %s\n' %
					(self.name, revision.rev), file=self.proj_tree.log_file)
...
class log_serializer(dependency_node):
	...
	def set_revision_to_dump(self, revision, log_revs, need_dump, has_nodes):
		self.hg_revision = revision.hg_revision
	...
	def do_dump(self):
		...
		if self.hg_revision is not None and self.need_dump:
			self.hg_revision.print(self.log_output_file)
			self.hg_revision = None
		...
		if node.action == b'delete':
			...
			if delete_rev_id is not None:
				...
				if delete_revision is not None:
					delete_branch = delete_revision.branch
					if delete_branch is branch:
						delete_branch.delete(self.HEAD())
						self.head_branch = None
						return self.empty_tree
					delete_branch.delete(self.HEAD(), quiet=True)
			return base_tree
			...
			if branch.delete_if_merged:
				...
				if merged_at_rev is not None:
					if merged_at_rev.branch is branch:
						# The branch is empty
						print('Branch "%s;%d" deleted because it doesn\'t have changess'
							% (branch.name, merged_at_rev.rev), file=self.log_file)
					else:
						# Deleting this branch because it's been merged
						print('Deleting the branch "%s" because it has been merged to branch "%s" at rev %d'
							% (branch.name, merged_at_rev.branch.name, merged_at_rev.rev),
								file=self.log_file)
					continue
		...
		for rev_info in self.deleted_revs:
			...
			if merged_at_rev is not None:
				if merged_at_rev.branch is rev_info.branch:
					...
					continue
				print('Deleted revision %s on branch "%s" has been merged to branch "%s" at rev %s'
					% (rev_info.rev, rev_info.branch.name, merged_at_rev.branch.name, merged_at_rev.rev),
					file=self.log_file)
				continue
