...
class branch_map:
	def __init__(self, cfg, name, refname, revisions_ref=None):
		self.cfg = cfg
		self.name_match = glob_match(name, cfg.replacement_vars, match_dirs=False, match_files=False, capture=True)

		if refname:
			self.refname_sub = glob_expand(refname, cfg.replacement_vars, self.name_match)

			if revisions_ref:
				self.revs_ref_sub = glob_expand(revisions_ref, cfg.replacement_vars, self.name_match)
			else:
...
class project_config:
	...
	def load(self, xml_node):
		...
		for node in xml_node.findall("./*"):
			...
			elif tag == 'MergeBranch':
				self.add_branch_merge_node(node)
			elif tag == 'Replace':
		...
		self.paths.append(xml_node.get('Branch', self.name), vars_dict=self.replacement_vars)
	...
	## Add regex patterns and substitutions to convert HG branch names
	# to refnames.
	...
	def add_branch_map_node(self, branch_map_node):

		node = branch_map_node.find("./Branch")
		if node is None:
			raise Exception_cfg_parse("Missing <Branch> node in <MapBranch>")

		branch = node.text
		if not branch:
			raise Exception_cfg_parse("Missing branch name pattern in <MapBranch><Branch> node")

		node = branch_map_node.find("./Refname")
		if node is not None:
			refname = node.text
		else:
			refname = None
		...
		if refname:
			node = branch_map_node.find("./RevisionRef")
		...
		new_map = branch_map(self, branch, refname, revs_ref)

		if new_map.key() in self.map_set:
			if branch_map_node.get('FromDefault') is None:
				raise Exception_cfg_parse("Branch mapping for '%s' specified twice in the config" % new_map.name_match.globspec)
	...
	def add_path_copy_node(self, path_copy_node):
		...
		node = path_copy_node.find("./RevId")
		if node is not None:
			rev = node.text
			if not rev:
				raise Exception_cfg_parse("Missing revision identifier in <CopyPath><RevId> node")
		else:
		...
		node = path_copy_node.find("./FromRevId")
		if node is not None:
			from_rev = node.text
			if not from_rev:
				raise Exception_cfg_parse("Missing revision identifier in <CopyPath><FromRevId> node")
		else:
			node = path_copy_node.find("./FromRev")
			if node is None:
				raise Exception_cfg_parse('<CopyPath> requires <FromRev> or <FromRevId>" specifier')
			...
			if type(from_rev) is int and rev < from_rev:
				raise Exception_cfg_parse("In <CopyPath> specification, <FromRev> needs to be less or equal than <Rev>")

		self.add_revision_action(rev,
				history_revision_action(b'copy', path, copyfrom_path=from_path, copyfrom_rev=from_rev))
		return

	def add_branch_merge_node(self, path_merge_node):

		node = path_merge_node.find("./RevId")
		if node is not None:
			rev = node.text
			if not rev:
				raise Exception_cfg_parse("Missing revision identifier in <MergeBranch><RevId> node")
		else:
			node = path_merge_node.find("./Rev")
			if node is None:
				raise Exception_cfg_parse('<MergeBranch> requires <FromRev> or <FromRevId>" specifier')

			rev = node.text
			if not rev:
				raise Exception_cfg_parse("Missing revision number in <MergeBranch><Rev> node")
			try:
				rev = int(rev)
			except ValueError:
				raise Exception_cfg_parse("Invalid revision number '%s' in <MergeBranch><Rev> node" % (rev))

		node = path_merge_node.find("./FromRevId")
		if node is not None:
			from_rev = node.text
			if not from_rev:
				raise Exception_cfg_parse("Missing revision identifier in <MergeBranch><FromRevId> node")
		else:
			node = path_merge_node.find("./FromRev")
			if node is None:
				raise Exception_cfg_parse("Missing <FromRev> node in <MergeBranch>")

			from_rev = node.text
			if not from_rev:
				raise Exception_cfg_parse("Missing revision number in <MergeBranch><FromRev> node")
			try:
				from_rev = int(from_rev)
			except ValueError:
				raise Exception_cfg_parse("Invalid revision number '%s' in <MergeBranch><FromRev> node" % (from_rev))

			if type(from_rev) is int and rev < from_rev:
				raise Exception_cfg_parse("In <MergeBranch> specification, <FromRev> needs to be less or equal than <Rev>")

		self.add_revision_action(rev,
				history_revision_action(b'merge', '', kind=b'dir', copyfrom_path='', copyfrom_rev=from_rev))
		return
	...
	def process_delete_file(self, node):
		path = node.get('Path')
		if not path:
			raise Exception_cfg_parse('<DeleteFile> should have Path="<path>" attribute')

		# Check if the path matches the project path
		if node.get('FromDefault') != 'Yes' and not self.paths.match(path, True):
			raise Exception_cfg_parse('<DeleteFile> Path="%s"> doesn\'t match the project node Path' % (path))
		...
		if rev is None:
			rev = node.get("RevId", None)
			if not rev:
				raise Exception_cfg_parse('<DeleteFile requires Rev="<rev>" or RevId="<rev>" attribute')
	...
	def process_formatting_node(self, node):
		...
		try:
			...
			fmt.fix_last_eol = bool_property_value(node, "FixLastEOL", fmt.fix_eol)
	...
	<IgnoreFiles>.hgtags;.hgsigs</IgnoreFiles>
