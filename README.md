...
Suppose, you ran the program with `hg_repo1/`, producing Git repository `git_repo1/`.
You have the the second repository `hg_repo2/`,
...
A `<SkipCommit>` specification is ignored if this revision is tagged, or it's producing a merge commit,
...
A legacy codebase before advent of `.editorconfig` and other style enforcement tools could become quite disheveled.
...
and `<MapBranch>` specifications.
`<MapBranch>` specifications are processed first, then sections in `<Project>`,
...
It defaults to the value of `TrimWhitespace` attribute.
...
if it ends without one. It defaults to the value of `FixEOL` attribute.
...
the program continues processing revisions from the source repository and queuing the hashing operations.
...
The commits for different branches can complete out of order with their revision numbers.
...
The source repository reading by the main thread normally completes before all commits are done.
